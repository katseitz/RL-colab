...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/katharinaseitz/Documents/projects/year_1/RL-colab/RL_in-scanner_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    get_ready_text = visual.TextStim(win=win, name='get_ready_text',
        text='Get Ready! The experimenter will start the game momentarily.',
        font='Arial',
        pos=(0, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    while continueRoutine:
        ...
        if advance_press_pd.status == STARTED and not waitOnFlip:
            theseKeys = advance_press_pd.getKeys(keyList=list(pointer), ignoreKeys=["escape"], waitRelease=True)
    ...
    for thisPractice_loop in practice_loop:
        ...
        prac_left_text.setText(left_prac_text)
        ...
        while continueRoutine:
            ...
            if cue_resp_lp.status == STARTED and not waitOnFlip:
                theseKeys = cue_resp_lp.getKeys(keyList=list(prac_left_button), ignoreKeys=["escape"], waitRelease=True)
        ...
        while continueRoutine and routineTimer.getTime() < 2.5:
            ...
            if thisExp.status == PAUSED:
                pauseExperiment(
                    ...
                    win=win, 
                    timers=[routineTimer, globalClock], 
                    currentRoutine=prac_left_response,
                )
                # skip the frame we paused on
                continue
        ...
        prac_right_text.setText(right_prac_text)
        ...
        while continueRoutine:
            ...
            if cue_resp_rp.status == STARTED and not waitOnFlip:
                theseKeys = cue_resp_rp.getKeys(keyList=list(prac_right_button), ignoreKeys=["escape"], waitRelease=True)
        ...
        while continueRoutine and routineTimer.getTime() < 2.5:
            ...
            if thisExp.status == PAUSED:
                pauseExperiment(
                    ...
                    win=win, 
                    timers=[routineTimer, globalClock], 
                    currentRoutine=prac_right_response,
                )
                # skip the frame we paused on
                continue
    ...
    while continueRoutine:
        ...
        if advance_to_runs.status == STARTED and not waitOnFlip:
            theseKeys = advance_to_runs.getKeys(keyList=list(pointer), ignoreKeys=["escape"], waitRelease=True)
        ...
        if thisExp.status == PAUSED:
            pauseExperiment(
                ...
                win=win, 
                timers=[routineTimer, globalClock], 
                currentRoutine=summary_instructions,
            )
            # skip the frame we paused on
            continue
    ...
    for thisRun in runs:
        ...
        while continueRoutine:
            ...
            if thisExp.status == PAUSED:
                pauseExperiment(
                    ...
                    win=win, 
                    timers=[routineTimer, globalClock], 
                    currentRoutine=scanner_trigger,
                )
                # skip the frame we paused on
                continue
        ...
        for thisTrial in trials:
            ...
            #since trial number ranges from 0-150 on sheet
            trial_num = trials.thisN + 50*runs.thisN 
            
            ISI = jitters.loc[(jitters['Run'] == runs.thisN + 1) & (jitters['Trial'] == trial_num + 1)]["ISI"].item() / 1000
            ITI = jitters.loc[(jitters['Run'] == runs.thisN + 1) & (jitters['Trial'] == trial_num + 1)]["ITI"].item() / 1000
            ...
            while continueRoutine and routineTimer.getTime() < 1.5:
                ...
                if cue_resp.status == STARTED and not waitOnFlip:
                    theseKeys = cue_resp.getKeys(keyList=list(button_list), ignoreKeys=["escape"], waitRelease=True)
                ...
                if all_keys.status == STARTED and not waitOnFlip:
                    theseKeys = all_keys.getKeys(keyList=None, ignoreKeys=["escape"], waitRelease=True)
            ...
            # Run 'End Routine' code from jitters_code
            ...
            currentLoop.addData('cueRespOnTime', fmriClock.getTime())
            ...
            currentLoop.addData('cueRespOffTime', fmriClock.getTime())
            ...
            currentLoop.addData('outcomeOnTime', fmriClock.getTime())
            #if no press is made
            ...
            currentLoop.addData('outcomeOffTime', fmriClock.getTime())
            ...
            continueRoutine = True
            # update component parameters for each repeat
            # Run 'Begin Routine' code from log_times_code
            currentLoop.addData('fixOnTime', fmriClock.getTime())
            ...
            # Run 'End Routine' code from log_times_code
            currentLoop.addData('fixOffTime', fmriClock.getTime())
        ...
        while continueRoutine:
            ...
            if thisExp.status == PAUSED:
                pauseExperiment(
                    ...
                    win=win, 
                    timers=[routineTimer, globalClock], 
                    currentRoutine=leftover_time_break,
                )
                # skip the frame we paused on
                continue
